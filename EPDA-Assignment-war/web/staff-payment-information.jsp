<%-- 
    Document   : staff-payment-information
    Created on : May 13, 2024, 6:36:15 PM
    Author     : TheOne
--%>
...
<html>
      ...
      <div class="min-h-full">
        ...
        <header class="bg-white shadow">
          <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
            <h1 class="text-3xl font-bold tracking-tight text-gray-900">Payment's Information</h1>
          </div>
        </header>
        <main>
                  ...
                  <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                    <table class="min-w-full divide-y divide-gray-200">
                      <thead class="bg-gray-50">
                        <tr>
                          ...
                          <th scope="col" class="relative px-6 py-3">
                            <span class="sr-only">Collect Payment</span>
                          </th>
                          <th scope="col" class="relative px-6 py-3">
                      ...
                      <tbody class="divide-y divide-gray-200">
                        <% 
                            List<AgcPayment> paymentList = null;
                            if (request.getAttribute("paymentList") != null) {
                                paymentList = (List<AgcPayment>) request.getAttribute("paymentList");
                            }
                            if (paymentList != null) {
                                for (AgcPayment agcPayment : paymentList) {
                        %>
                        <tr class="bg-white">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900"><%= agcPayment.getId() %></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900"><%= agcPayment.getClassId() %></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900"><%= agcPayment.getCustomerId() %></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900"><%= agcPayment.getAmount() %></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900"><%= agcPayment.getStatus() %></td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <% if (agcPayment.getStatus().equals("pending")) { %>
                                    <a href="StaffPaymentCollect?id=<%= agcPayment.getId() %>" class="text-indigo-600 hover:text-indigo-900">Collect Payment</a>
                                <% } %>
                            </td>
                            
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <a href="#" onclick="confirmDelete('<%= agcPayment.getId() %>')" class="text-red-600 hover:text-red-900">Delete</a>
                            </td>
                        </tr>
                        <% 
                                }
        ...
        <script>
            function confirmDelete(id) {
                if (confirm('Are you sure you want to delete this payment?')) {
                    window.location.href = 'StaffPaymentDelete?id=' + id;
                }
            };
    ...
    <script>
        alert('<%= session.getAttribute("popupMessage") %>');
    </script>
