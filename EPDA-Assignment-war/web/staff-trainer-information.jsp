<%-- 
    Document   : staff-tranier
    Created on : May 11, 2024, 9:05:40 PM
    Author     : TheOne
--%>
      ...
      <div class="min-h-full">
        ...
        <header class="bg-white shadow">
          <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
            <h1 class="text-3xl font-bold tracking-tight text-gray-900">Trainer's Information</h1>
          </div>
        </header>
        <main>
                  ...
                  <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                    <table class="min-w-full divide-y divide-gray-200">
                      ...
                      <tbody class="divide-y divide-gray-200">
                        <% 
                            List<AgcTrainer> trainerList = null;
                            if (request.getAttribute("trainerList") != null) {
                                trainerList = (List<AgcTrainer>) request.getAttribute("trainerList");
                            }
                            if (trainerList != null) {
                                for (AgcTrainer trainer : trainerList) {
                        %>
                        <tr class="bg-white">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900"><%= trainer.getId() %></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900"><%= trainer.getName() %></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900"><%= trainer.getEmail() %></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900"><%= trainer.getPhone() %></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900"><%= trainer.getIs_approve() %></td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">  
                                <a href="StaffTrainerEdit?id=<%= trainer.getId() %>" class="text-indigo-600 hover:text-indigo-900">Edit</a>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <a href="#" onclick="confirmDelete('<%= trainer.getId() %>')" class="text-red-600 hover:text-red-900">Delete</a>
                            </td>
                        </tr>
                        <% 
                                }
        ...
        <script>
            function confirmDelete(id) {
                if (confirm('Are you sure you want to delete this trainer?')) {
                    window.location.href = 'StaffTrainerDelete?id=' + id;
                }
            };
